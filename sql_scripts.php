...
function getPhase5_SE1_FichaSocioEconomica(): array
{
    return [
        [
            'nome' => 'Criar temp CPFs',
            'sql' => "CREATE TABLE {DEST}.DBO._temp_cpf (cpf CHAR(14))"
        ],
        [
            'nome' => 'Coletar CPFs ADQ1 (mttbcon)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT c.ADQ1_CPFCGC FROM {ORIG}.DBO.mttbcon c WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = c.CODEMP AND X.REGIAO = c.REGIAO AND X.NUCLEO = c.NUCLEO AND X.CONTRATO = c.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs ADQ2 (mttbcon)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT c.ADQ2_CPF FROM {ORIG}.DBO.mttbcon c WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = c.CODEMP AND X.REGIAO = c.REGIAO AND X.NUCLEO = c.NUCLEO AND X.CONTRATO = c.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs ADQ3 (mttbcon)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT c.ADQ3_CPF FROM {ORIG}.DBO.mttbcon c WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = c.CODEMP AND X.REGIAO = c.REGIAO AND X.NUCLEO = c.NUCLEO AND X.CONTRATO = c.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs ADQ4 (mttbcon)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT c.ADQ4_CPF FROM {ORIG}.DBO.mttbcon c WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = c.CODEMP AND X.REGIAO = c.REGIAO AND X.NUCLEO = c.NUCLEO AND X.CONTRATO = c.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs DATU_CGC_CPF (mttbcon)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT c.DATU_CGC_CPF FROM {ORIG}.DBO.mttbcon c WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = c.CODEMP AND X.REGIAO = c.REGIAO AND X.NUCLEO = c.NUCLEO AND X.CONTRATO = c.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs DATU_AD2 (mttbcon)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT c.DATU_AD2_CPF FROM {ORIG}.DBO.mttbcon c WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = c.CODEMP AND X.REGIAO = c.REGIAO AND X.NUCLEO = c.NUCLEO AND X.CONTRATO = c.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs DATU_AD3 (mttbcon)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT c.DATU_AD3_CPF FROM {ORIG}.DBO.mttbcon c WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = c.CODEMP AND X.REGIAO = c.REGIAO AND X.NUCLEO = c.NUCLEO AND X.CONTRATO = c.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs DATU_AD4 (mttbcon)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT c.DATU_AD4_CPF FROM {ORIG}.DBO.mttbcon c WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = c.CODEMP AND X.REGIAO = c.REGIAO AND X.NUCLEO = c.NUCLEO AND X.CONTRATO = c.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs AD1 (mttbhis)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT h.AD1_CGCCPF FROM {ORIG}.DBO.mttbhis h WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = h.CODEMP AND X.REGIAO = h.REGIAO AND X.NUCLEO = h.NUCLEO AND X.CONTRATO = h.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs AD2 (mttbhis)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT h.AD2_CPF FROM {ORIG}.DBO.mttbhis h WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = h.CODEMP AND X.REGIAO = h.REGIAO AND X.NUCLEO = h.NUCLEO AND X.CONTRATO = h.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs AD3 (mttbhis)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT h.AD3_CPF FROM {ORIG}.DBO.mttbhis h WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = h.CODEMP AND X.REGIAO = h.REGIAO AND X.NUCLEO = h.NUCLEO AND X.CONTRATO = h.CONTRATO)"
        ],
        [
            'nome' => 'Coletar CPFs AD4 (mttbhis)',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf SELECT DISTINCT h.AD4_CPF FROM {ORIG}.DBO.mttbhis h WHERE EXISTS (SELECT 1 FROM {DEST}.DBO.CON_FIDC X WHERE X.CODEMP = h.CODEMP AND X.REGIAO = h.REGIAO AND X.NUCLEO = h.NUCLEO AND X.CONTRATO = h.CONTRATO)"
        ],
        [
            'nome' => 'Criar temp CPFs normalizados',
            'sql' => "CREATE TABLE {DEST}.DBO._temp_cpf_final (cpf CHAR(14))"
        ],
        [
            'nome' => 'Normalizar CPFs',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cpf_final SELECT DISTINCT RIGHT('00000000000000' + LTRIM(RTRIM(t.cpf)), 14) FROM {DEST}.DBO._temp_cpf t"
        ],
        [
            'nome' => 'Ficha Socioeconomica (SE1)',
            'sql' => "SELECT s.CODEMP, s.CGCCPF, s.FISJUR, s.NOMTIT, s.END_TITC, s.CPL_ENDC, s.BAIRROC, s.CIDADEC, s.ESTADOC, s.COD_CEPC, s.CPL_CEPC, s.CXA_POSC, s.DDD_TELC, s.NUM_TELC, s.RAM_TELC, s.DDD_FAXC, s.NUM_FAXC, s.CODNAC, s.CODNAT, s.CODSEX, s.ESTCIV, s.REGCAS, s.DTANAS, s.ATVPRO, s.NUMIDT, s.ORGIDT, s.ESTIDT, s.FXA_REND, s.CNJNOM, s.CNJCPF, s.CODUSR, s.DTAIMP, s.DTAMOV, s.EMISS_CONJUG, s.CODAGENTE, s.EMAIL, s.PIS, s.PIS_CONJ, s.CONS_ORGAO, s.CONS_SETOR, s.CONS_MUNIC, s.CONS_MATR, s.LCOB_BANCO, s.LCOB_AGENCIA, s.LCOB_CONTA, s.LCOB_TIPO_CC
                      INTO {DEST}.DBO.ficha_socio_economica
                      FROM {ORIG}.DBO.mttbse1 s
                      INNER JOIN {DEST}.DBO._temp_cpf_final t ON t.cpf = s.CGCCPF
                      WHERE s.codemp IN ({CODEMP})"
        ],
        [
            'nome' => 'Limpar temp CPFs',
            'sql' => "DROP TABLE {DEST}.DBO._temp_cpf; DROP TABLE {DEST}.DBO._temp_cpf_final"
        ],
    ];
}

function getPhase6_DEP_SE2(): array
{
    return [
        [
            'nome' => 'Criar temp COD_ADQ',
            'sql' => "CREATE TABLE {DEST}.DBO._temp_cod_adq (codadq INT)"
        ],
        [
            'nome' => 'Coletar COD_ADQ_PRIN',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cod_adq SELECT DISTINCT COD_ADQ_PRIN FROM {DEST}.DBO.CADASTRO_FINANCIAMENTO"
        ],
        [
            'nome' => 'Coletar COD_COADQ1',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cod_adq SELECT DISTINCT COD_COADQ1 FROM {DEST}.DBO.CADASTRO_FINANCIAMENTO"
        ],
        [
            'nome' => 'Coletar COD_COADQ2',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cod_adq SELECT DISTINCT COD_COADQ2 FROM {DEST}.DBO.CADASTRO_FINANCIAMENTO"
        ],
        [
            'nome' => 'Coletar COD_COADQ3',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cod_adq SELECT DISTINCT COD_COADQ3 FROM {DEST}.DBO.CADASTRO_FINANCIAMENTO"
        ],
        [
            'nome' => 'Criar temp COD_ADQ final',
            'sql' => "CREATE TABLE {DEST}.DBO._temp_cod_adq_final (codadq INT)"
        ],
        [
            'nome' => 'Distinct COD_ADQ',
            'sql' => "INSERT INTO {DEST}.DBO._temp_cod_adq_final SELECT DISTINCT t.codadq FROM {DEST}.DBO._temp_cod_adq t"
        ],
        [
            'nome' => 'Dependentes do Cliente (DEP)',
            'sql' => "SELECT s.*
                      INTO {DEST}.DBO.DEPENDENTES_CLIENTE
                      FROM {ORIG}.DBO.mttbdep s
                      INNER JOIN {DEST}.DBO._temp_cod_adq_final t ON t.codadq = s.cliente_unic
                      WHERE s.codemp IN ({CODEMP}) AND t.codadq <> 0"
        ],
        [
            'nome' => 'Cadastro Inscricoes (SE2)',
            'sql' => "SELECT s.*
                      INTO {DEST}.DBO.CADASTRO_INSCRICOES
                      FROM {ORIG}.DBO.mttbse2 s
                      INNER JOIN {DEST}.DBO._temp_cod_adq_final t ON t.codadq = s.cliente_unic
                      WHERE s.codemp IN ({CODEMP})"
        ],
        [
            'nome' => 'Limpar temp COD_ADQ',
            'sql' => "DROP TABLE {DEST}.DBO._temp_cod_adq; DROP TABLE {DEST}.DBO._temp_cod_adq_final"
        ],
    ];
}
